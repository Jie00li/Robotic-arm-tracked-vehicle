...
typedef struct
{
  __I  uint32_t CPUID;                        /*!< Offset: 0x00  CPU ID Base Register 中央处理器 ID 寄存器             */
  __IO uint32_t ICSR;                         /*!< Offset: 0x04  Interrupt Control State Register 中断控制状态寄存器   */
  __IO uint32_t VTOR;                         /*!< Offset: 0x08  Vector Table Offset Register 矢量表偏移寄存器         */
  __IO uint32_t AIRCR;                        /*!< Offset: 0x0C  Application Interrupt / Reset Control Register 应用中断/复位控制寄存器*/
  __IO uint32_t SCR;                          /*!< Offset: 0x10  System Control Register 系统控制寄存器                */
  __IO uint32_t CCR;                          /*!< Offset: 0x14  Configuration Control Register 配置控制寄存器         */
  __IO uint8_t  SHP[12];                      /*!< Offset: 0x18  System Handlers Priority Registers (4-7, 8-11, 12-15) 系统处理程序优先级寄存器 （4-7、8-11、12-15）*/
  __IO uint32_t SHCSR;                        /*!< Offset: 0x24  System Handler Control and State Register 系统处理程序控制和状态寄存器*/
  __IO uint32_t CFSR;                         /*!< Offset: 0x28  Configurable Fault Status Register 可配置故障状态寄存器*/
  __IO uint32_t HFSR;                         /*!< Offset: 0x2C  Hard Fault Status Register 硬故障状态寄存器           */
  __IO uint32_t DFSR;                         /*!< Offset: 0x30  Debug Fault Status Register 调试故障状态寄存器        */
  __IO uint32_t MMFAR;                        /*!< Offset: 0x34  Mem Manage Address Register 内存管理地址寄存器        */
  __IO uint32_t BFAR;                         /*!< Offset: 0x38  Bus Fault Address Register 总线故障地址寄存器         */
  __IO uint32_t AFSR;                         /*!< Offset: 0x3C  Auxiliary Fault Status Register 辅助故障状态寄存器    */
  __I  uint32_t PFR[2];                       /*!< Offset: 0x40  Processor Feature Register 处理器功能寄存器           */
  __I  uint32_t DFR;                          /*!< Offset: 0x48  Debug Feature Register 调试功能寄存器                 */
  __I  uint32_t ADR;                          /*!< Offset: 0x4C  Auxiliary Feature Register 辅助功能寄存器             */
  __I  uint32_t MMFR[4];                      /*!< Offset: 0x50  Memory Model Feature Register 内存模型功能寄存器      */
  __I  uint32_t ISAR[5];                      /*!< Offset: 0x60  ISA Feature Register ISA 功能注册                     */
} SCB_Type;                                                
...
typedef struct
{
  ...
  __I  uint32_t CALIB;                        /*!< Offset: 0x0C  SysTick Calibration Register        */
	} SysTick_Type;//定义一个名为SysTick_Type的结构体存放四个寄存器
 ...
 *
 * @param   ticks   number of ticks between two interrupts 在两个中断之间的时间间隔（有多少个Systick时钟周期）
 ...
 */
static __INLINE uint32_t SysTick_Config(uint32_t ticks)//开启Systick中断，以及配置中断间隔时间
{ 
  if (ticks > SysTick_LOAD_RELOAD_Msk)  return (1);            /* Reload value impossible 假如数值超过范围则无法重新计时*/
                                                               
  SysTick->LOAD  = (ticks & SysTick_LOAD_RELOAD_Msk) - 1;      /* set reload register 设置重新加载寄存器（减1是因为装载也算一个时钟周期）*/
  NVIC_SetPriority (SysTick_IRQn, (1<<__NVIC_PRIO_BITS) - 1);  /* set Priority for Cortex-M0 System Interrupts 设置 Cortex-M0 系统中断的优先级*/
  SysTick->VAL   = 0;                                          /* Load the SysTick Counter Value 加载系统滴答计数器值*/
  SysTick->CTRL  = SysTick_CTRL_CLKSOURCE_Msk | 
                   SysTick_CTRL_TICKINT_Msk   | 
                   SysTick_CTRL_ENABLE_Msk;                    /* Enable SysTick IRQ and SysTick Timer 启用 SysTick IRQ 和 SysTick Timer*/
