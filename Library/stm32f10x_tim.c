...
void TIM_TimeBaseInit(TIM_TypeDef* TIMx, TIM_TimeBaseInitTypeDef* TIM_TimeBaseInitStruct)
{
  ...
  /* Generate an update event to reload the Prescaler and the Repetition counter
     values immediately 生成更新事件以重新加载预分频器和重复计数器立即值*/
...
/**
  ...
  *   This parameter can be one of the following values:
  *     @arg TIM_PSCReloadMode_Update: The Prescaler is loaded at the update event.预分频器在更新事件时加载。
  *     @arg TIM_PSCReloadMode_Immediate: The Prescaler is loaded immediately.预分频器将立即加载。
  * @retval None
  */
