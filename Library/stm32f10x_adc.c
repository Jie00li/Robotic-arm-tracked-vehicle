...
void ADC_Init(ADC_TypeDef* ADCx, ADC_InitTypeDef* ADC_InitStruct)
{
  ...
  /* Check the parameters */
  assert_param(IS_ADC_ALL_PERIPH(ADCx));
  assert_param(IS_ADC_MODE(ADC_InitStruct->ADC_Mode));//ADC选择
  assert_param(IS_FUNCTIONAL_STATE(ADC_InitStruct->ADC_ScanConvMode));//扫描转换模式（单通道或多通道）
  assert_param(IS_FUNCTIONAL_STATE(ADC_InitStruct->ADC_ContinuousConvMode));//数据转换模式
  assert_param(IS_ADC_EXT_TRIG(ADC_InitStruct->ADC_ExternalTrigConv));//外部触发源选择 
  assert_param(IS_ADC_DATA_ALIGN(ADC_InitStruct->ADC_DataAlign));//数据对齐方式 
  assert_param(IS_ADC_REGULAR_LENGTH(ADC_InitStruct->ADC_NbrOfChannel));//规则组转换通道的数目
...
uint16_t ADC_GetConversionValue(ADC_TypeDef* ADCx)
{
  ...
  return (uint16_t) ADCx->DR;//直接读取DR数据寄存器
}
...
/**
  ...
  *   This parameter can be one of the following values:
  *     @arg ADC_FLAG_AWD: Analog watchdog flag模拟看门狗标志位
  *     @arg ADC_FLAG_EOC: End of conversion flag(EOC)规则组转换完成标志位
  ...
  *     @arg ADC_FLAG_STRT: Start of regular group conversion flag规则组开始转换标志位
